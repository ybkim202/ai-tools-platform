...
| DB | PostgreSQL 15 | PostgreSQL 17 (**Supabase 호스팅**, `ap-northeast-2`, 2026-06-30 Render 무료 만료로 이전) ✅ | `backend/load_tools_fixed.py:13` |
...
- **[해소]** 배포 타깃 확정 — **백엔드 Railway · DB Supabase · 프론트 Vercel**. (DB는 2026-06-30 Render 무료 만료로 Supabase `ap-northeast-2`(프로젝트 `grepity`)로 이전. Supabase 무료는 장기 유휴 시 pause·영구삭제 없음)
- **[해소]** 운영 DB 실적재 확인됨 — 복원 검증 시 tools **111**·pricing 99·tags 19·tool_tags 312·benchmarks 16·**news 155**·**github_trending 95**. 뉴스·깃헙트렌드도 cron 수집으로 점등 완료(만료 전 누적분 pg_dump 백업→새 DB 복원).
