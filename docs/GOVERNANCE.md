...
- **2026-06-30** DB 호스팅 Render→**Supabase** 이전 — Render 무료 PostgreSQL 만료로 운영 DB 중단. 만료 직전 `pg_dump` 백업(111도구·뉴스155·트렌드95)을 새 DB로 `pg_restore` 복원 후 `DATABASE_URL` 교체. (초기 복원은 Neon에 했다가 무료 지속성 고려해 **Supabase**(`ap-northeast-2`, 프로젝트 `grepity`)로 확정.) **재발 방지**: ① 무료 DB는 만료/한도 있음 → `backend/backup_db.py`(pg_dump 풀백업) 도입, ② 운영 누적분이 레포 시드에 없던 갭 → `backend/export_seeds.py`(라이브→시드 baseline)로 `tools_data.json` 78→111 갱신, ③ `pg_dump`/`pg_restore`는 **클라이언트 버전 ≥ 서버(PG17+)** 필요(`brew install libpq`), ④ Supabase는 **Session pooler(5432, IPv4)** 사용 — direct(`db.<ref>.supabase.co`)는 IPv6 전용이라 IPv4 환경 실패, 조직당 무료 2개 한도. 현 토폴로지: **백엔드 Railway · DB Supabase · 프론트 Vercel**.
